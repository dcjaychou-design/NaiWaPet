...
请从 [NaiWaPet 1.0.0 发布页面](https://github.com/dcjaychou-design/NaiWaPet/releases/tag/v1.0.0) 下载：

- `NaiWaPet-1.0.0-win-x64.exe`：推荐，下载后直接双击运行，无需安装。
- `NaiWaPet-1.0.0-win-x64-portable.zip`：压缩包版本，解压后运行 `NaiWaPet.exe`。

当前应用版本为 **1.0.0**，支持 Windows 11 x64。

目前发布物未做商业代码签名，Windows SmartScreen 可能在第一次运行时显示“未知发布者”。请只从上述发布页面下载，并用同名 `.sha256` 文件核对哈希。发布页面中的 EXE、ZIP、校验文件和说明均由 `v1.0.0` 标签对应的同一份源码自动生成。
...
发布程序位于 `artifacts/`。打包脚本会把 `1.0.0` 同步写入程序版本信息和发布文件名。更完整的说明见 [docs/BUILDING.md](docs/BUILDING.md)。
